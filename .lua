...
local function toast_notify(msg)
	spawn(function()
...
toggleButton.MouseButton1Click:Connect(function()
	if c and c.Parent then
		resReq()
		lp.Character = c
		toast_notify("attempting to respawn...")
	else
		toast_notify("Loading...")
		c = lp.Character
		if not (c and c.Parent) then
			toast_notify("error: character not found")
			return nil
		end
		local hum = c:FindFirstChildOfClass("Humanoid")
		if not (hum and hum.Parent) then
			toast_notify("error: humanoid not found")
			return nil
		end
		if hum.Health == 0 then
			toast_notify("error: humanoid health 0")
			return nil
		end
		resReq()
		lp.Character = c
		wait(5.1)
		if not (c and c.Parent) then
			toast_notify("error: character removed")
			return nil
		end
		if not (hum and hum.Parent) then
			toast_notify("error: humanoid removed")
			return nil
		end
		if hum.Health == 0 then
			toast_notify("error: humanoid died")
			return nil
		end
		c:BreakJoints()
		hum.Health = 0
